...
class ThreadForDwindow(QThread):
    ...
    def __init__(self,cfg):
        super().__init__()
        self.cfg = cfg
        self.interact = PetIneraction()
    def run(self) -> None:
        if self.cfg.chatmode == "free":
            time.sleep(1)
            self.toMain.emit("meowcat: {}".format(random.choice(cat_words)))
        if self.cfg.chatmode == "recordDone":
            res, text = self.interact.step()
            if (res is not None) and (text is not None):
                self.toMain.emit("master: {}".format(text))
                self.toMain.emit("meowcat: {}".format(res))
...
class DisplayWindow(QDockWidget, Ui_DockWidget):
    ...
    def addComment(self,msg):
        '''
        在主线程里添加，否则显示会延迟，但消息多时可能会导致窗口无响应。
        '''
